...
def _resolve_variant_subset(
    ...
    requested_names = list(
        controller.get("variant_names", [])
        or controller.get("fast_variant_names", [])
        or []
    )
...
def _slice_bars_for_runtime(
    bars: Any, controller_state: dict[str, Any] | None = None
) -> Any:
    controller = dict(controller_state or {})
...
def run_candidate_universe_labeling_diagnostic(
    ...
    *,
        ...
        rows.append(
            {
                ...
                "label_summary": {
                    ...
                    "positive_rate": (
                        float(labels_df["label"].mean()) if not labels_df.empty else 0.0
                    ),
                    "avg_pnl_r": (
                        float(labels_df["pnl_r"].mean()) if not labels_df.empty else 0.0
                    ),
                },
                "model_metrics": (
                    model_summary.to_dict()
                    if model_summary
                    else {"status": "no_labels"}
                ).get("metrics", {}),
                "feature_audit": {
    ...
    output_dir = (
        REPORTS_DIR / "runs" / run_id / "candidate_universe_labeling_diagnostic"
    )
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "summary.json").write_text(
        json.dumps(payload, indent=2, default=str), encoding="utf-8"
...
def _generate_custom_candidates(
    bars: Any, cfg: Stage2Config, zones: list[BNRZone], variant: dict[str, Any]
) -> list[CandidateSetup]:
    pd = _require_pandas()
    ...
    for zone in zones:
        ...
        for break_ts, break_row in one_minute.iterrows():
            ...
            if (
                ...
                > pd.Timestamp(str(variant["latest_trigger_time"])).time()
            ):
                continue
            for direction in ["long", "short"]:
                if counts[direction] >= max_per_direction:
                    continue
                if not _break_condition(
                    break_row,
                    zone,
                    direction,
                    str(variant["break_mode"]),
                    cfg.break_buffer_points,
                ):
                    continue
...
def _break_condition(
    row: Any, zone: BNRZone, direction: str, mode: str, buffer_points: float
) -> bool:
...
def _lineage_record(
    candidate: CandidateSetup, variant: dict[str, Any]
) -> dict[str, Any]:
...
def _deduplicate(
    ...
    return kept, {
        ...
        "duplicate_cluster_count": sum(
            1 for items in grouped.values() if len(items) > 1
        ),
...
def _variant_summary(
    ...
    return {
        ...
        "candidate_count_delta_vs_baseline": (
            len(records) - len(baseline) if baseline else 0
        ),
        "new_deduped_candidates_vs_baseline": (
            len(keys - baseline_keys) if baseline else 0
        ),
...
def _effective_sample_size(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    by_session_direction = Counter(
        f"{row['session_date']}|{row['direction']}" for row in records
    )
    session_cluster_ess = sum(
        1.0 / count for count in by_session.values() for _ in range(count)
    )
    direction_cluster_ess = sum(
        1.0 / count for count in by_session_direction.values() for _ in range(count)
    )
    return {
        "raw_n": len(records),
        "session_cluster_ess": float(session_cluster_ess),
        "session_direction_cluster_ess": float(direction_cluster_ess),
        "avg_cluster_size_session": (
            float(mean(by_session.values())) if by_session else 0.0
        ),
        "avg_cluster_size_session_direction": (
            float(mean(by_session_direction.values())) if by_session_direction else 0.0
        ),
    }
...
def _select_next_stage(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    eligible = [
        row
        for row in rows
        if row["governance_decision"] == "eligible_for_labeling_diagnostic"
    ]
    ranked = sorted(
        eligible,
        key=lambda row: (
    ...
    return [
        {
            "variant": row["variant"],
            "candidate_count": row["candidate_count"],
            "session_direction_cluster_ess": row["effective_sample_size"][
