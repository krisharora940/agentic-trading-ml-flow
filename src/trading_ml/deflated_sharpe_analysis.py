...
def build_deflated_sharpe_audit(
    ...
    *,
    ...
    feature_map = {
        ...
        for row in stage2_result.get("features_records", [])
    }
    merged_records = []
    for row in stitched:
        merged = dict(row)
    ...
    threshold = float(
        overrides.get(
            "decision_threshold",
            controller.get("frozen_threshold", frozen.get("threshold", 0.45)),
        )
        or 0.45
    )
    sizing_policy = str(
        overrides.get(
            ...
            controller.get(
                ...
                frozen.get("sizing_policy", "binary_threshold_v1"),
            ),
        )
    )
    regime_throttle_policy = str(
        overrides.get(
            ...
            controller.get(
                ...
                frozen.get("regime_throttle_policy", "none"),
            ),
        )
    )
    regime_size_policy = str(
        overrides.get(
            "regime_size_policy",
            controller.get(
                "benchmark_regime_size_policy", frozen.get("regime_size_policy", "none")
            ),
        )
    )
    ...
    if observed_sr is None:
        return {
            "status": "pending",
            "reason": "insufficient_trade_returns",
    ...
    if sr_std is None or sr_std <= 0:
        return {
            "status": "pending",
            "reason": "insufficient_sharpe_dispersion",
            "trade_count": len(trade_returns),
        }
...
def _sharpe_dispersion(
    cpcv: dict[str, Any], search_results: dict[str, Any]
) -> tuple[float | None, str]:
    path_sharpes = [
        float(row.get("sharpe_r", 0.0) or 0.0)
        for row in cpcv.get("paths", [])
        if row.get("sharpe_r") is not None
    ]
    if len(path_sharpes) >= 2:
        return _sample_std(path_sharpes), "cpcv_path_sharpe"
    trial_proxies = [
        float(row.get("net_avg_pnl_r", 0.0) or 0.0)
        for row in search_results.get("ranked_trials", [])
    ]
