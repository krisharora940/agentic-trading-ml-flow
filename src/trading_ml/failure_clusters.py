...
def build_failure_clusters(
    attempts: list[dict[str, Any]], *, limit: int = 5
) -> list[dict[str, Any]]:
    try:
        import pandas as pd
    except ImportError:
    ...
    if "features" in frame.columns:
        ...
        frame = pd.concat(
            [
                frame.drop(columns=["features"]).reset_index(drop=True),
    ...
    grouped = failed.groupby(
        ["failure_reason", "setup_state", "environment_state", "time_bucket"],
        dropna=False,
    )
    for (failure_reason, setup_state, environment_state, time_bucket), group in grouped:
        ...
        cluster = FailureCluster(
            ...
            avg_probability=(
                float(group["probability"].mean()) if "probability" in group else 0.0
            ),
            ...
            evidence={
                "avg_break_efficiency_ratio": _mean(
                    group, "feature_break_efficiency_ratio"
                ),
                "avg_reclaim_close_location": _mean(
                    group, "feature_reclaim_close_location"
                ),
                "avg_deepest_zone_retrace_fraction": _mean(
                    group, "feature_deepest_zone_retrace_fraction"
                ),
                "avg_post_reclaim_close_strength": _mean(
                    group, "feature_post_reclaim_close_strength"
                ),
