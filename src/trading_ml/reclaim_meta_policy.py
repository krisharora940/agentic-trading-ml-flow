...
def get_reclaim_meta_policies() -> list[dict[str, Any]]:
    return [
        {"name": "baseline_gated"},
        {
            ...
            "apply_subtypes": ["balanced_reclaim_continuation", "clean_break_continuation"],
...
def apply_reclaim_meta_policy(records: list[dict[str, Any]], *, policy_name: str) -> list[dict[str, Any]]:
    policy = next(policy for policy in get_reclaim_meta_policies() if policy["name"] == policy_name)
    ...
    for record in records:
        ...
        if in_scope:
            if "min_reclaim_close_location" in policy:
                if float(record.get("reclaim_close_location", 0.0) or 0.0) < float(policy["min_reclaim_close_location"]):
                    continue
            if "min_post_reclaim_close_strength" in policy:
                if float(record.get("post_reclaim_close_strength", 0.0) or 0.0) < float(policy["min_post_reclaim_close_strength"]):
                    continue
            if "min_reclaim_body_strength" in policy:
                if float(record.get("reclaim_body_strength", 0.0) or 0.0) < float(policy["min_reclaim_body_strength"]):
                    continue
