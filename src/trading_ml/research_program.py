...
DEFAULT_SEARCH_BUDGETS = {
    "setup": {"max_trials": 12, "max_cycles": 1, "max_runtime": "bounded_by_batch", "allowed_knobs": ["earliest_trigger_time", "horizon_bars", "target_multiple", "break_buffer_points"], "disallowed_knobs": ["holdout data", "post-hoc threshold edits"]},
    "model": {"max_trials": 2, "max_cycles": 1, "max_runtime": "bounded_by_batch", "allowed_knobs": ["model_family"], "disallowed_knobs": ["label edits", "holdout data"]},
    "feature": {"max_trials": 4, "max_cycles": 2, "max_runtime": "bounded_by_batch", "allowed_knobs": ["feature_family"], "disallowed_knobs": ["holdout data", "trial-specific thresholds"]},
    "threshold": {"max_trials": 5, "max_cycles": 1, "max_runtime": "bounded_by_batch", "allowed_knobs": ["decision_threshold"], "disallowed_knobs": ["feature edits", "label edits", "holdout data"]},
    "label": {"max_trials": 8, "max_cycles": 1, "max_runtime": "bounded_by_batch", "allowed_knobs": ["horizon_bars", "stop_multiple", "target_multiple"], "disallowed_knobs": ["holdout data", "model escalation"]},
    "sample_expansion": {"max_trials": 4, "max_cycles": 1, "max_runtime": "bounded_by_batch", "allowed_knobs": ["earliest_trigger_time", "latest_trigger_time", "break_buffer_points"], "disallowed_knobs": ["holdout data", "model escalation", "threshold-only edits"]},
    "subtype": {"max_trials": 6, "max_cycles": 2, "max_runtime": "bounded_by_batch", "allowed_knobs": ["setup_subtype"], "disallowed_knobs": ["exact clock filters", "holdout data"]},
    "policy_gate": {"max_trials": 4, "max_cycles": 1, "max_runtime": "bounded_by_batch", "allowed_knobs": ["break_quality_policy"], "disallowed_knobs": ["exact clock filters", "holdout data"]},
    "policy_meta": {"max_trials": 4, "max_cycles": 1, "max_runtime": "bounded_by_batch", "allowed_knobs": ["reclaim_meta_policy"], "disallowed_knobs": ["exact clock filters", "holdout data"]},
    "tail_path_cleanup": {"max_trials": 4, "max_cycles": 1, "max_runtime": "bounded_by_batch", "allowed_knobs": ["structural tail-path BNR cleanup policies"], "disallowed_knobs": ["model changes", "threshold-only search", "holdout data"]},
    "exit_behavior_research": {"max_trials": 8, "max_cycles": 1, "max_runtime": "bounded_existing_artifact_replay", "allowed_knobs": ["point-in-time exit behavior families"], "disallowed_knobs": ["entry changes", "model training", "holdout data", "path-specific exits"]},
    "candidate_universe_expansion": {"max_trials": 8, "max_cycles": 1, "max_runtime": "bounded_universe_diagnostic", "allowed_knobs": ["canonical candidate universe definitions"], "disallowed_knobs": ["model training", "holdout data", "undocumented discretionary filters"]},
    "validation_window": {"max_trials": 1, "max_cycles": 1, "max_runtime": "single_reserved_run", "allowed_knobs": ["boundary source"], "disallowed_knobs": ["spec edits"]},
    "holdout_confirmation": {"max_trials": 1, "max_cycles": 1, "max_runtime": "single_holdout_run", "allowed_knobs": ["holdout source"], "disallowed_knobs": ["any benchmark edits"]},
}
...
KILL_CRITERIA = {
    ...
    "model": ["kill model escalation while CPCV is failing", "kill if simpler baseline has equal or better utility"],
    "feature": ["kill feature line after 2 cycles with no utility or CPCV improvement"],
    "threshold": ["kill if losses are already concentrated in the high-confidence bucket", "kill if threshold lift is only lower breadth"],
    "label": ["kill if calibration or mean path PnL deteriorates"],
    "sample_expansion": ["kill if expanded sample keeps the same CPCV tail paths", "kill if added breadth materially lowers mean path PnL"],
    "subtype": ["kill if tail improves but mean path PnL deteriorates", "kill if subtype support is sample-limited"],
    "policy_gate": ["kill if gate is tied to a narrow clock artifact", "kill if breadth falls below benchmark tolerance"],
    "policy_meta": ["kill if binary utility exceeds sized/filtered utility across cycles"],
    "tail_path_cleanup": ["kill current BNR benchmark if the same CPCV tail paths persist after structural cleanup", "kill cleanup policy if trade count falls below 70% of benchmark"],
    "exit_behavior_research": ["kill if bounded exit replay cannot improve PBO and drawdown together", "kill if improvement is driven by path-specific or non-point-in-time behavior"],
    "candidate_universe_expansion": ["kill if lineage, deduplication, or point-in-time audit fails", "kill if effective sample size does not improve despite raw expansion"],
...
def _next_step_plan(state: dict[str, Any], program_gaps: list[str]) -> dict[str, Any]:
    ...
    if walk_forward.get("status") == "pass" and cpcv.get("status") == "fail":
        ...
        return {
            ...
            "controller_override": {"active_family": family_decision["selected_family"]},
...
def _select_next_research_family(
    *,
    ...
    if persistent_tail.get("status") == "active":
        ...
        evidence["tail_cleanup_failed"] = bool(persistent_tail.get("tail_cleanup_failed"))
        evidence["label_geometry_failed"] = bool(persistent_tail.get("label_geometry_failed"))
        evidence["sample_expansion_failed"] = bool(persistent_tail.get("sample_expansion_failed"))
        evidence["used"]["persistent_tail_failure"] = persistent_tail
        evidence["not_used"].append("families that already failed the same CPCV tail paths")
    if _benchmark_exhaustion_triggered(persistent_tail):
        return _benchmark_exhaustion_decision(evidence=evidence, memory=memory, persistent_tail=persistent_tail)
    family_scores = [_score_family(family, evidence, memory) for family in RESEARCH_FAMILIES]
    ranked = sorted(family_scores, key=lambda row: (row["risk_adjusted_score"], row["diagnostic_alignment"]), reverse=True)
    ...
    selected = next((row for row in ranked if row["family"] not in exhausted), ranked[0])
    ...
    return {
        ...
        "why_rejected": {str(row["family"]): _rejection_reason(row, selected_family) for row in ranked[1:]},
...
def _build_family_evidence(
    *,
    ...
    return {
        ...
        "time_bucket_is_exact": bool(time_of_day.get("key")) and ":" in str(time_of_day.get("key")),
        "high_confidence_loss": probability_bucket.get("key") == "[0.65,1.00]" and float(probability_bucket.get("total_pnl_r", 0.0) or 0.0) < 0,
        "calibration_failed": calibration.get("status") == "fail",
        "reclaim_features_active": any(name in top_features for name in ["reclaim_body_strength", "post_reclaim_close_strength", "reclaim_close_location"]),
        "break_features_active": any(name in top_features for name in ["break_close_distance_to_zone", "break_efficiency_ratio", "first_break_close_excess_points"]),
    }


def _score_family(family: str, evidence: dict[str, Any], memory: dict[str, Any]) -> dict[str, Any]:
    scores = {
    ...
    if family in {"model", "holdout_confirmation"}:
        ...
        scores["evidence_support"] = 0.0 if cleanup_failed else (5.0 if persistent else 2.0)
        scores["expected_upside"] = 0.0 if cleanup_failed else (4.0 if persistent else 2.0)
        scores["diagnostic_alignment"] = 0.0 if cleanup_failed else (5.0 if persistent else 2.0)
        ...
        scores["evidence_support"] = 3.0 if high_conf_loss and calibration_failed else 1.0
        ...
        scores["evidence_support"] = 0.0 if label_failed else (4.0 if cleanup_failed else (3.0 if calibration_failed else 1.0))
        scores["expected_upside"] = 0.0 if label_failed else (4.0 if cleanup_failed else (3.0 if calibration_failed else 1.0))
        scores["diagnostic_alignment"] = 0.0 if label_failed else (5.0 if cleanup_failed else (4.0 if calibration_failed else 1.0))
        scores["overfit_risk"] = 5.0 if label_failed else 3.0
        scores["promotion_relevance"] = 0.0 if label_failed else (4.0 if cleanup_failed else 3.0)
        reason = (
        ...
        scores["evidence_support"] = 0.0 if expansion_failed else (5.0 if label_failed else (1.0 if persistent else 0.5))
        scores["expected_upside"] = 0.0 if expansion_failed else (4.0 if label_failed else (1.0 if persistent else 0.5))
        scores["diagnostic_alignment"] = 0.0 if expansion_failed else (5.0 if label_failed else (1.0 if persistent else 0.5))
        scores["overfit_risk"] = 5.0 if expansion_failed else 3.0
        scores["implementation_cost"] = 3.0
        scores["promotion_relevance"] = 0.0 if expansion_failed else (5.0 if label_failed else 1.0)
        reason = (
...
def _benchmark_exhaustion_decision(
    *,
    ...
    return {
        ...
        "why_selected": reason,
        "why_rejected": {family: "Benchmark is exhausted or structurally fragile under the hard kill rule." for family in RESEARCH_FAMILIES},
        "known_risks": ["continuing this exact benchmark definition likely burns compute on a structurally fragile tail"],
        ...
        "search_budget": {"max_trials": 0, "max_cycles": 0, "allowed_knobs": [], "disallowed_knobs": ["all knobs for this benchmark definition"]},
...
def _build_multi_cycle_memory(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if REGISTRY_PATH.exists():
        for line in REGISTRY_PATH.read_text(encoding="utf-8").splitlines()[-250:]:
            ...
            family = accounting.get("family") or _family_from_experiment_id(str(record.get("experiment_id", "")))
    ...
    exhausted = sorted({family for family, count in failure_counts.items() if count >= 3} | killed)
...
def _persistent_tail_failure_from_artifacts() -> dict[str, Any]:
    ...
    for artifact_dir in sorted(runs_root.glob("*/node_artifacts"), key=lambda path: path.stat().st_mtime)[-40:]:
        ...
        if not search_files or not audit_files:
            continue
        try:
            search = json.loads(search_files[-1].read_text(encoding="utf-8")).get("payload", {})
            audit = json.loads(audit_files[-1].read_text(encoding="utf-8")).get("payload", {})
        except json.JSONDecodeError:
            continue
        ...
        path_ids = [str(row.get("path_id")) for row in cpcv.get("worst_paths", [])[:3] if row.get("path_id")]
        if not path_ids:
            continue
        family = str(search.get("executed_research_family") or search.get("search_results", {}).get("family") or "unknown")
        observations.append(
            {
                "family": family,
                "batch_decision": str(search.get("search_results", {}).get("batch_decision") or ""),
                "accepted_trial": bool(search.get("search_results", {}).get("accepted_trial")),
                "dsr_status": str(dsr.get("status") or ""),
                "dsr_n_trials": int(dsr.get("n_trials", 0) or 0),
                "hard_gate_passed": cpcv.get("status") == "pass" and dsr.get("status") == "pass",
                "path_ids": path_ids,
    ...
    for obs in observations:
        bucket = signatures.setdefault(obs["signature"], {"families": set(), "runs": [], "path_ids": obs["path_ids"]})
    ...
    for signature, bucket in signatures.items():
        ...
        if len(families) >= 2:
            tail_cleanup_failed = any(
                obs["family"] == "tail_path_cleanup" and obs.get("batch_decision") != "accept"
                for obs in observations
                if obs.get("signature") == signature
            )
            label_geometry_failed = any(obs["family"] == "label" for obs in observations if obs.get("signature") == signature)
            sample_expansion_failed = any(obs["family"] == "sample_expansion" for obs in observations if obs.get("signature") == signature)
            matching = [obs for obs in observations if obs.get("signature") == signature]
            dsr_failed_after_real_trials = any(
                obs.get("dsr_status") == "fail" and int(obs.get("dsr_n_trials", 0) or 0) > 1
                for obs in matching
            )
            any_hard_gate_passed = any(bool(obs.get("hard_gate_passed")) for obs in matching)
            return {
                "status": "active",
                "signature": signature,
                ...
                "blocked_families": ["model", "threshold", "policy_meta", "validation_window", "holdout_confirmation"],
                "allowed_families": ["tail_path_cleanup", "subtype", "label", "sample_expansion", "policy_gate"],
            }
    return {"status": "inactive", "reason": "no_repeated_tail_signature", "observations": observations[-5:]}


def _family_from_experiment_id(experiment_id: str) -> str | None:
    for family in sorted(RESEARCH_FAMILIES, key=len, reverse=True):
        if f"-{family}-" in experiment_id or f"-{family.replace('_', '-')}-" in experiment_id:
            return family
    return None
...
def _budget_for_family(family: str) -> dict[str, Any]:
    ...
    budget["minimum_evidence_required"] = ["walk_forward pass", "purging pass", "CPCV improves or passes", "DSR/PSR uses full batch trial count"]
    return budget


def _approval_required(family: str) -> str | None:
    if family in {"setup", "model", "feature", "threshold", "label", "sample_expansion", "subtype", "policy_gate", "policy_meta", "tail_path_cleanup"}:
...
def _known_risks(family: str, evidence: dict[str, Any]) -> list[str]:
    ...
    if evidence.get("time_bucket_is_exact") and family in {"policy_gate", "policy_meta", "subtype"}:
    ...
    if family == "tail_path_cleanup":
        risks.append("cleanup must improve exact failed CPCV paths, not just headline utility")
    if family == "sample_expansion":
        risks.append("expanded breadth may add lower-quality setups and reduce mean path PnL")
    return risks
