...
def build_translation_analysis(
    ...
    *,
    ...
    sizing_name = sizing_policy or str(
        bnr_config.get("frozen_benchmark", {}).get(
            "sizing_policy", "binary_threshold_v1"
        )
    )
    throttle_name = regime_throttle_policy or str(
        bnr_config.get("frozen_benchmark", {}).get("regime_throttle_policy", "none")
    )
    regime_size_name = regime_size_policy or str(
        bnr_config.get("frozen_benchmark", {}).get("regime_size_policy", "none")
    )
            ...
            rows.append(
                {
                    ...
                    "throttled_signals": int(
                        execution.get("throttled_signals", 0) or 0
                    ),
                    "utility_score": utility.get("score"),
                    "binary_trade_count": int(
                        binary_execution.get("trade_count", 0) or 0
                    ),
                    "binary_total_pnl_r": float(
                        binary_execution.get("total_pnl_r", 0.0) or 0.0
                    ),
                    "binary_utility_score": binary_utility.get("score"),
                    "utility_gap_vs_binary": (
                        float(utility.get("score")) - float(binary_utility.get("score"))
            ...
            rows.append(
                {
                    ...
                    "win_rate": win_rate,
                    "total_pnl_r": (
                        float(selected["pnl_r"].sum()) if trade_count else 0.0
                    ),
                    "utility_score": None,
                }
            )
    ...
    best = max(
        ...
        key=lambda item: (
            (
                ...
                if item.get("utility_score") is not None
                else float("-inf")
            ),
            item["total_pnl_r"],
            item["trade_count"],
