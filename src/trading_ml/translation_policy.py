...
def compute_position_size(probability: float, *, threshold: float, policy_name: str | None) -> float:
    ...
    if kind == "confidence_linear":
        ...
        if threshold >= 1.0:
            return min_size
        progress = max(0.0, min(1.0, (probability - threshold) / max(1.0 - threshold, 1e-9)))
    ...
    if kind == "fractional_kelly_proxy":
        ...
        if threshold >= 1.0:
            return min_size
        normalized_edge = max(0.0, min(1.0, (probability - threshold) / max(1.0 - threshold, 1e-9)))
...
def allow_signal_for_regime(record: dict[str, Any], *, policy_name: str | None) -> bool:
    ...
    if "max_high_vol_state" in policy and float(record.get("reg_high_vol_state", 0.0) or 0.0) > float(policy["max_high_vol_state"]):
        return False
