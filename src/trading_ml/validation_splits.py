...
def build_walk_forward_splits(
    merged: Any,
) -> tuple[list[tuple[Any, Any, WalkForwardFold]], dict[str, Any]]:
    ...
    for fold_idx, start in enumerate(
        range(
            min_train_sessions, len(unique_sessions) - test_sessions + 1, step_sessions
        ),
        start=1,
    ):
        ...
        train = merged[
            merged["session_date"].astype(str).isin(train_sessions_list)
        ].copy()
        test = merged[
            merged["session_date"].astype(str).isin(test_sessions_list)
        ].copy()
