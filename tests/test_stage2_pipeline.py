...
class Stage2PipelineTests(unittest.TestCase):
    def test_bnr_zone_candidate_label_and_feature_audit_are_point_in_time(self) -> None:
        index = pd.date_range("2026-01-02 09:30:00", periods=12, freq="30s", tz="America/New_York")
        bars = pd.DataFrame(
            {
                ...
                "volume": [1000, 900, 1200, 1100, 1000, 950, 925, 900, 880, 860, 840, 820],
            },
            index=index,
        )
        ...
        candidates = generate_breakout_candidates(bars, zones, earliest_trigger_time="09:32:00")
        self.assertGreaterEqual(len(candidates), 1)
        self.assertGreaterEqual(pd.Timestamp(candidates[0].decision_time), pd.Timestamp(zones[0].decision_available_at))
