...
class ValidationAuditTests(unittest.TestCase):
    ...
    def test_validation_audit_surfaces_walk_forward_purging_and_multiple_testing(
        self,
    ) -> None:
        state = build_agent_loop_state()
        stage2_result = run_stage2_research_engine(
            Stage2Config(**state["stage2_config"])
        )
        ...
        self.assertIn(
            audit["walk_forward"].get("backend", "custom"),
            {"custom", "ml4t_diagnostic"},
        )
        ...
        self.assertIn(
            audit["random_signal_plumbing"]["status"], {"pass", "fail", "pending"}
        )
