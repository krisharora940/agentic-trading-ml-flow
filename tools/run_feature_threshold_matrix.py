...
def main() -> None:
    ...
    feature_families = list(
        bnr_config["feature_threshold_search_v1"]["space"]["feature_family"]
    )
    thresholds = [
        float(x)
        for x in bnr_config["feature_threshold_search_v1"]["space"][
            "decision_threshold"
        ]
    ]
            ...
            rows.append(
                {
                    ...
                    "walk_forward_status": validation.get("walk_forward", {}).get(
                        "status"
                    ),
                    ...
                    "max_drawdown_r": float(
                        execution.get("max_drawdown_r", 0.0) or 0.0
                    ),
