...
def _build_session_slice(
    ...
    output = (
        DATA_DIR
        / "cache"
        / f"{symbol.lower()}_{timeframe}_failure_map_slice_{max_sessions}sessions.parquet"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build a larger-sample failure map for the current baseline research stack."
    )
    ...
    if args.gate_policy:
        ...
        execution = run_event_driven_policy_backtest(
            selected_records, threshold=args.threshold
        )
