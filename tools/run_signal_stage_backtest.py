...
def main() -> None:
    ...
    for family in families:
        ...
        random_total = (
            float(random_execution.get("total_pnl_r", 0.0) or 0.0)
            if random_execution.get("status") == "complete"
            else None
        )
        total = (
            float(execution.get("total_pnl_r", 0.0) or 0.0)
            if execution.get("status") == "complete"
            else None
        )
        rows.append(
            {
                ...
                "ic_to_sharpe_proxy": (
                    (total / abs(float(execution.get("max_drawdown_r", 1.0) or 1.0)))
