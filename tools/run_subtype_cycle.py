...
def _build_session_slice(source_path: str, *, max_sessions: int, symbol: str, timeframe: str, timezone: str) -> str:
    ...
    output = DATA_DIR / "cache" / f"{symbol.lower()}_{timeframe}_subtype_slice_{max_sessions}sessions.parquet"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a governed BNR subtype cycle on the active Stage 2 source or a sliced subset.")
    parser.add_argument("--max-sessions", type=int, default=0, help="If set, build a smaller parquet slice using the first N sessions.")
    ...
    ranked = sorted(
        rows,
        key=lambda row: (float(row["utility_score"] or float("-inf")), row["total_pnl_r"]),
        reverse=True,
    )
    payload = {
